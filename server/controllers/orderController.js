...
export const createOrder = async (req, res) => {
  try {
    ...
    const token = await Notification.find({ userID: userID });
    if (!token) {
      console.log("No Token");
    }
    // console.log("token on backend", token[0]);

    const message = {
      notification: {
        title: "Order Confirmed 🎉",
        body: `Your order for ${subCategory} has been placed successfully!`,
      },
      token: token[0].token, //
    };
    ...
    res.status(201).json({
      message: "Order created & notification sent!",
      order: newOrder,
    });
  } catch (error) {
